...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"AFK!**\n➢ `{DEFAULTUSER}` Sedang Off,, Jangan Ganggu Dulu Ya Babu..!\
        \n╰▻Reason  :  `{string}`")
    else:
        await afk_e.edit(f"AFK!**\n➢ `{DEFAULTUSER}` Sedang Off,, Jangan Ganggu Dulu Ya Babu..!")
    if user.last_name: 
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond(f"➢Oke Kontoll,, `{DEFAULTUSER}` Telah Aktif Kembali..!")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"➢Cari Apa Kawan? `{DEFAULTUSER}` Sedang Off..!\n➟Durasi Afk : {afk_since} Yang Lalu.\
                        \n╰▻Reason  :  `{AFKREASON}`")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"➢Cari Apa Kawan? `{DEFAULTUSER}` Sedang Off..!\n➟Durasi Afk : {afk_since} Yang Lalu.\
                        \n╰▻Reason  :  `{AFKREASON}`")
                else:
